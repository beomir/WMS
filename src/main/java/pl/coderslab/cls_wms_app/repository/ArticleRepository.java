...
public interface ArticleRepository extends JpaRepository<Article, Long> {
    ...
    @Query("Select a from Article a left join Stock s on s.article.id = a.id where a.production = true and a.active = true and a.company = ?1 and a.productionArticle.warehouse.name = ?2")
    Article articleForProduction(Long id);

    @Query(value = "SELECT a.article_number article_number,a.article_desc,sum(s.pieces_qty) volume,case when pa.quantity_for_finished_product is null then ia.quantity_for_finished_product else pa.quantity_for_finished_product end depth,case when ia.production_article_type is null then pa.production_article_type else ia.production_article_type end changeBy FROM article a LEFT JOIN storage s ON a.id = s.article_id left join intermediate_article ia on a.id = ia.article_id left join production_article pa on a.id = pa.article_id join intermediate_article_production_article iapa on pa.id = iapa.production_article_id join company c on a.company_id = c.id left outer join warehouse w on pa.warehouse_id = w.id where a.production = true and a.active = true and c.name = ?1 and w.name = ?2 and iapa.production_article_id = ( select distinct iapa.production_article_id from article a inner join production_article ia on a.id = ia.article_id inner join intermediate_article_production_article iapa on ia.id = iapa.production_article_id where a.id = ?3) group by a.article_number,case when pa.quantity_for_finished_product = null then ia.quantity_for_finished_product else pa.quantity_for_finished_product end ,a.article_desc UNION SELECT a.article_number,a.article_desc,sum(s.pieces_qty),case when pa.quantity_for_finished_product is null then ia.quantity_for_finished_product else pa.quantity_for_finished_product end , case when ia.production_article_type is null then pa.production_article_type else ia.production_article_type end FROM article a RIGHT JOIN storage s ON a.id = s.article_id left join intermediate_article ia on a.id = ia.article_id left join production_article pa on a.id = pa.article_id join intermediate_article_production_article iapa on ia.id = iapa.intermediate_article_id join company c on a.company_id = c.id right outer join warehouse w on ia.warehouse_id = w.id where a.production = true and a.active = true and c.name = ?1 and w.name = ?2 and iapa.production_article_id = ( select distinct iapa.production_article_id from article a inner join production_article ia on a.id = ia.article_id inner join intermediate_article_production_article iapa on ia.id = iapa.production_article_id where a.id = ?3 ) group by a.article_number,case when pa.quantity_for_finished_product = null then ia.quantity_for_finished_product else pa.quantity_for_finished_product end ,a.article_desc order by 1 ",nativeQuery = true)
