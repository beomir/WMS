...
public interface ReceptionRepository extends JpaRepository<Reception, Long> {
    ...
    @Modifying
    @Transactional
    @Query(value = "INSERT INTO storage (created, hd_number, last_update, pieces_qty, quality, article_id, company_id, status_id, unit_id, warehouse_id, change_by,comment) select localtime,hd_number,localtime,pieces_qty,quality,article_id,company_id,status_id,unit_id,warehouse_id, change_by,concat('Reception number: ',reception_number) from receptions where reception_number = (?1);",nativeQuery = true)
